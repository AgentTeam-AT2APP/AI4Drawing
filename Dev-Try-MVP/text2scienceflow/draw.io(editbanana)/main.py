...
class Pipeline:
    ...
    def _prepare_input_image(self, input_path: str, output_dir: str) -> str:
        """Normalize input to an image path. PDF inputs are rasterized to page 1 PNG."""
        ext = Path(input_path).suffix.lower()
        if ext != ".pdf":
            return input_path

        pdf_name = Path(input_path).stem
        png_path = os.path.join(output_dir, f"{pdf_name}_page1.png")
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"PDF 转 PNG 失败，请确认已安装 ghostscript(gs): {stderr}")
        if not os.path.exists(png_path):
            raise RuntimeError("PDF 转换完成但未找到输出 PNG 文件")

        print(f"   [输入] 已将 PDF 第1页转换为图片: {png_path}")
        return png_path
    ...
    def process_image(self,
        ...
        normalized_image_path = self._prepare_input_image(image_path, img_output_dir)
        
        print("\n[0] Preprocess...")
        processed_image_path, was_upscaled, scale_factor = self._preprocess_image(normalized_image_path, img_output_dir)
...
Examples:
  ...
  python main.py -i test.png --groups image arrow
    ...
    parser.add_argument("-i", "--input", type=str, 
                        help="输入图片/PDF路径（不指定则处理input/目录下所有文件）")
