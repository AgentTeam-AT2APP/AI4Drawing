...
class Sam3InfoExtractor(BaseProcessor):
    ...
    def process(self, context: ProcessingContext) -> ProcessingResult:
        ...
        # 在缺权重/模型加载失败时做最小兜底：整图作为一个 picture 元素，
        # 以便主流程仍可输出可编辑 DrawIO XML。
        try:
            self.load_model()
        except Exception as e:
            self._log(f"SAM3模型加载失败，启用兜底路径: {e}")
            fallback = ElementInfo(
                id=0,
                element_type="picture",
                bbox=BoundingBox(
                    x1=0,
                    y1=0,
                    x2=context.canvas_width,
                    y2=context.canvas_height,
                ),
                score=1.0,
                polygon=[
                    [0, 0],
                    ...
                    [0, context.canvas_height],
                ],
                source_prompt="fallback_no_sam3_model",
            )
            context.elements = [fallback]
        ...
        for group_type in process_order:
            ...
            try:
                ...
                self._log(f"SAM3推理失败，跳过该组 {group_type.value}: {e}")
                continue
        ...
        # 若所有组都为空，兜底整图为一个 picture，避免主流程中断
        if not all_elements:
            self._log("SAM3未产出有效元素，使用整图兜底元素")
            all_elements = [
                ElementInfo(
                    id=0,
                    element_type="picture",
                    ...
                    source_prompt="fallback_no_sam3_detection",
                )
            ]
