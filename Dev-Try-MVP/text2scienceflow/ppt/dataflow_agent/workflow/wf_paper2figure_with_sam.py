...
def create_p2fig_graph() -> GenericGraphBuilder:  # noqa: N802
    ...
    def _get_abstract_intro(state: Paper2FigureState):
        ...
        content = text.strip()
        # 控制上下文体积，避免超长论文全文导致首个 LLM 节点耗时过长或超时。
        max_chars = 12000
    ...
    async def figure_desc_generator_node(state: Paper2FigureState) -> Paper2FigureState:
        ...
        图标提示词生成器节点
        """
        model_name = getattr(state.request, "fig_desc_model", None) or "gpt-5.1"
        figure_desc_generator = create_react_agent(
    ...
    async def figure_generator_node(state: Paper2FigureState) -> Paper2FigureState:
        ...
        """
        fig_desc_result = (state.agent_results.get("figure_desc_generator") or {}).get("results", {})
        prompt = fig_desc_result.get("fig_desc", {})
        if not prompt:
            prompt = (
                "Create a clean scientific workflow diagram with 6-8 connected modules, "
                "minimal text labels, clear arrows, and high visual contrast."
            )
        safe_prompt = json.dumps(prompt, ensure_ascii=False) if isinstance(prompt, (dict, list)) else str(prompt)
        ...
        await generate_or_edit_and_save_image_async(
            ...
            api_url=state.request.image_api_url or state.request.chat_api_url,
            api_key=state.request.image_api_key or state.request.chat_api_key or os.getenv("SILICONFLOW_API_KEY") or os.getenv("DF_API_KEY") ,
        ...
        try:
            ...
            # 某些文生图模型仅支持 generation，不支持 edit；这里回退为复用初稿图，保证流程可继续。
            log.error(f"[figure_generator] edit failed, fallback to draft image: {e}")
            shutil.copy(save_path, layout_save_path)
    ...
    async def figure_layout_sam_node(state: Paper2FigureState) -> Paper2FigureState:
        ...
        try:
            ...
            try:
                ...
                # 严格模式：不允许在 checkpoint 缺失/不完整时跳过 SAM。
                ckpt_path_obj = Path(sam_ckpt)
                ckpt_valid = ckpt_path_obj.exists() and ckpt_path_obj.stat().st_size > 100 * 1024 * 1024
                if not ckpt_valid:
                    raise FileNotFoundError(
                        f"[figure_layout_sam] Local SAM checkpoint missing or incomplete: {sam_ckpt}. "
                        "SAM step is required and cannot be skipped."
                    )
            ...
            log.error(f"[figure_layout_sam] Critical Failure: {e}")
            raise
    ...
    async def figure_mask_generator_node(state: Paper2FigureState) -> Paper2FigureState:
        ...
        try:
            ...
            # 1) 优先调用 HTTP MinerU 递归处理
            # 2) 失败时回退到本地 CLI MinerU（pipeline+cpu），避免 8010 不可用时直接中断
            try:
                ...
                log.error(f"[figure_mask] HTTP MinerU failed, fallback to local CLI MinerU: {e}")
                mineru_items = await recursive_run_mineru(
                    img_path=img_path,
                    out_dir=out_dir,
                    max_depth=max_depth,
                    current_depth=0,
            ...
            if len(mineru_items) <= details:
                ...
                for layout_idx, layout_it in enumerate(layout_items):
                    ...
                    try:
                        ...
                        # HTTP 失败时，回退到本地 CLI MinerU（不递归）
                        try:
                            sub_blocks = await recursive_run_mineru(Path(sub_path), sub_dir, max_depth=0)
                        except Exception as e2:
                            log.error(f"[figure_mask] 子图 CLI MinerU 解析失败 layout_idx={layout_idx}, path={sub_path}: {e2}")
                            continue
    ...
    def set_entry_node(state: Paper2FigureState) -> str:
        resume_from = getattr(state.request, "resume_from", "") or ""
        if resume_from:
            valid = {
            ...
            if resume_from in valid:
                log.warning(f"[resume] resume_from={resume_from}")
                return resume_from
            log.error(f"[resume] invalid resume_from={resume_from}, fallback to normal flow")
        if getattr(state.request, "skip_idea_steps", False):
            log.warning("skip_idea_steps=True, 直接进入 figure_generator")
    ...
    def _wrap_with_checkpoint(fn, node_name: str):
        async def _inner(state: Paper2FigureState) -> Paper2FigureState:
            state = await fn(state)
            save_p2f_checkpoint(state, node_name)
            return state
        return _inner

    nodes = {
        "_start_": _init_result_path,
        "paper_idea_extractor": _wrap_with_checkpoint(paper_idea_extractor_node, "paper_idea_extractor"),
        "figure_desc_generator": _wrap_with_checkpoint(figure_desc_generator_node, "figure_desc_generator"),
        "figure_generator": _wrap_with_checkpoint(figure_generator_node, "figure_generator"),
        "figure_layout_sam": _wrap_with_checkpoint(figure_layout_sam_node, "figure_layout_sam"),
        "figure_mask_generator": _wrap_with_checkpoint(figure_mask_generator_node, "figure_mask_generator"),
        "figure_icon_bg_remover": _wrap_with_checkpoint(figure_icon_bg_remover_node, "figure_icon_bg_remover"),
        "figure_ppt_generator": _wrap_with_checkpoint(figure_ppt_generation_node, "figure_ppt_generator"),
