...
def _normalize_chat_api_url(raw_url: str | None) -> str:
    """
    兼容两种配置写法：
    1) Base URL:  https://xxx/v1
    2) Endpoint:  https://xxx/v1/chat/completions
    内部统一为 Base URL，避免调用方重复拼接路径。
    """
    if not raw_url:
        return "test"
    url = raw_url.rstrip("/")
...
def _normalize_image_api_url(raw_url: str | None) -> str:
    """
    兼容两种配置写法：
    1) Base URL:  https://xxx/v1
    2) Endpoint:  https://xxx/v1/images/generations 或 https://xxx/v1/images/edits
    内部统一为 Base URL，避免调用方重复拼接路径。
    """
    if not raw_url:
        return "test"
...
class MainRequest:
    ...
    # ② LLM 接口
    chat_api_url: str = _normalize_chat_api_url(
        os.getenv("SILICONFLOW_API_URL") or os.getenv("DF_API_URL", "test")
    )
    api_key: str = os.getenv("SILICONFLOW_API_KEY") or os.getenv("DF_API_KEY", "test")
    chat_api_key: str = os.getenv("SILICONFLOW_API_KEY") or os.getenv("DF_API_KEY", "test") #没区别，但是不想改之前代码了；

    # ②.5 图像接口（优先 UNIAPI）
    image_api_url: str = _normalize_image_api_url(
        os.getenv("UNIAPI_API_URL")
        or os.getenv("SILICONFLOW_API_URL")
        or os.getenv("DF_API_URL", "test")
    )
    image_api_key: str = (
        os.getenv("UNIAPI_API_KEY")
        or os.getenv("SILICONFLOW_API_KEY")
        or os.getenv("DF_API_KEY", "test")
    )

    # ③ 选用的 LLM 名称
    model: str = (
        os.getenv("SILICONFLOW_MODEL_NAME")
        or os.getenv("DF_MODEL_NAME", "Pro/moonshotai/Kimi-K2.5")
    )
...
class DFRequest(MainRequest):
    ...
    embedding_model_name: str = os.getenv("DF_EMBEDDING_MODEL_NAME", "text-embedding-3-small")
...
class Paper2FigureState(MainState):
    ...
    # 断点续跑状态（仅 paper2figure_with_sam 使用）
    run_id: str = ""
