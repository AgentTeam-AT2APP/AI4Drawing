...
def _json_default(obj: Any) -> Any:
    """Best-effort JSON serializer for checkpointing."""
    try:
        return obj.__dict__
    except Exception:
        return str(obj)

def save_p2f_checkpoint(state: Any, node_name: str) -> None:
    """
    Persist Paper2FigureState checkpoint to disk.
    Explicit resume only: always write to result_path/checkpoints.
    """
    try:
        result_path = getattr(state, "result_path", "")
        if not result_path:
            return
        ckpt_dir = Path(result_path) / "checkpoints"
        ckpt_dir.mkdir(parents=True, exist_ok=True)
        state.last_completed_node = node_name
        ckpt_file = ckpt_dir / f"{node_name}.json"
        ...
        log.info(f"[checkpoint] saved: {ckpt_file}")
    except Exception as e:
        log.error(f"[checkpoint] save failed at node={node_name}: {e}")

def load_p2f_checkpoint(result_path: str) -> Dict[str, Any] | None:
    """
    Load latest Paper2FigureState checkpoint from disk.
    ...
    try:
        ...
        log.error(f"[checkpoint] load failed: {e}")
        return None
...
async def run_mineru(image_path: Path, output_dir: Path) -> bool:
    """调用 mineru 并返回是否成功"""
    env = os.environ.copy()
    # Force UTF-8 to avoid UnicodeDecodeError in some Python environments (e.g. Anaconda).
    env["PYTHONUTF8"] = "1"
