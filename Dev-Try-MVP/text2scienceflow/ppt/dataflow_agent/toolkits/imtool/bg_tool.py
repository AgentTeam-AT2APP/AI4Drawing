...
# Default to a directory path; AutoModelForImageSegmentation expects a model folder.
MODEL_PATH = CURRENT_DIR / "models" / "RMBG-2.0"
...
def ensure_model(model_path: Path) -> None:
    ...
    """
    # Treat a file path as its parent directory for model storage.
    is_dir_target = model_path.suffix == "" or model_path.is_dir()
    target_dir = model_path if is_dir_target else model_path.parent

    if is_dir_target and target_dir.exists() and any(target_dir.iterdir()):
        print(f"模型已存在: {target_dir}")
        return
    if (not is_dir_target) and model_path.exists():
    ...
    # Prefer modelscope CLI; fallback to python -m modelscope.cli
    modelscope_bin = shutil.which("modelscope")
    if modelscope_bin:
        cmd = [modelscope_bin, "download", "--model", "AI-ModelScope/RMBG-2.0", "--local_dir", str(target_dir)]
    else:
        cmd = ["python", "-m", "modelscope.cli", "download", "--model", "AI-ModelScope/RMBG-2.0", "--local_dir", str(target_dir)]
    ...
    if result.returncode != 0:
        raise FileNotFoundError(
            f"模型下载失败：命令执行错误 (code={result.returncode}).\n"
            f"cmd: {' '.join(cmd)}"
        )
    ...
    if is_dir_target:
        if not target_dir.exists() or not any(target_dir.iterdir()):
            raise FileNotFoundError(
                f"模型下载失败：未找到 {target_dir}。\n"
                "请检查 ModelScope 或手动下载。"
            )
        print(f"模型已成功下载到: {target_dir}")
    else:
