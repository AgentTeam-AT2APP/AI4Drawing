...
_SAM_OFFICIAL_CHECKPOINT_URLS: Dict[str, str] = {
    # Official SAM checkpoints (Meta public files)
    "sam_b.pt": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth",
    "sam_l.pt": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_l_0b3195.pth",
    "sam_h.pt": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_h_4b8939.pth",
    "sam_vit_b_01ec64.pth": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth",
    "sam_vit_l_0b3195.pth": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_l_0b3195.pth",
    "sam_vit_h_4b8939.pth": "https://dl.fbaipublicfiles.com/segment_anything/sam_vit_h_4b8939.pth",
}

_SAM_MIN_SIZE_BYTES: Dict[str, int] = {
    # sam_vit_b_01ec64.pth is about 357MB; use a conservative lower bound.
    "sam_b.pt": 100 * 1024 * 1024,
}
...
def _official_sam_url_for_checkpoint(checkpoint_path: Path) -> str | None:
    """
    根据 checkpoint 文件名返回官方 SAM 下载链接。
    """
    return _SAM_OFFICIAL_CHECKPOINT_URLS.get(checkpoint_path.name)


def _download_file(url: str, dst_path: Path) -> None:
    """
    下载文件到目标路径（先写临时文件，完成后原子替换）。
    """
    dst_path.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = dst_path.with_suffix(dst_path.suffix + ".downloading")
...
def _ensure_sam_checkpoint(checkpoint: str) -> str:
    """
    确保 SAM checkpoint 存在；若不存在则尝试自动下载官方权重。

    下载地址优先级：
    1) 环境变量 DF_SAM_CHECKPOINT_URL
    2) 根据 checkpoint 文件名匹配官方 URL
    """
    ckpt_path = Path(checkpoint).expanduser()
    min_size = _SAM_MIN_SIZE_BYTES.get(ckpt_path.name, 0)
    if ckpt_path.exists():
        if min_size <= 0 or ckpt_path.stat().st_size >= min_size:
            return str(ckpt_path.resolve())
        # Incomplete checkpoint: remove and redownload.
        try:
            ckpt_path.unlink()
        except Exception:
            # If deletion fails, proceed and let download overwrite via temp file.
            pass

    custom_url = os.getenv("DF_SAM_CHECKPOINT_URL", "").strip()
    url = custom_url or _official_sam_url_for_checkpoint(ckpt_path)
    if not url:
        raise FileNotFoundError(
            f"SAM checkpoint not found: {ckpt_path}. "
            "Please place the file manually, or set DF_SAM_CHECKPOINT_URL."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to auto-download SAM checkpoint from {url} to {ckpt_path}: {e}"
        ) from e

    if not ckpt_path.exists():
        raise FileNotFoundError(f"SAM checkpoint download finished but file not found: {ckpt_path}")
    return str(ckpt_path.resolve())
...
def _get_sam_model(
    ...
    ckpt_path = _ensure_sam_checkpoint(checkpoint)
    key = ckpt_path
    if key not in _SAM_MODELS:
        ...
        _SAM_MODELS[key] = UltralyticsSAM(ckpt_path)
...
def free_sam_model(checkpoint: str = "sam_b.pt") -> None:
    ...
    global _SAM_MODELS
    raw_key = checkpoint
    resolved_key = str(Path(checkpoint).expanduser().resolve())

    m = _SAM_MODELS.pop(raw_key, None)
    if m is None:
        m = _SAM_MODELS.pop(resolved_key, None)
    if m is not None:
