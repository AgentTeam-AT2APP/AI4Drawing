...
def _normalize_openai_base_url(api_url: str) -> str:
    """
    接受 base URL 或完整 endpoint，统一返回 base URL。
    """
...
async def _post_chat_completions(
    ...
    """
    url = f"{_normalize_openai_base_url(api_url)}/chat/completions".rstrip("/")
    return await _post_raw(url, api_key, payload, timeout)

def _is_dalle_model(model: str) -> bool:
    """
    判断是否为走 OpenAI images API 风格的模型
    """
    m = model.lower()
    return (
        m.startswith(("dall-e", "dall-e-2", "dall-e-3"))
        or "qwen-image" in m
        or "flux." in m
        or "kolors" in m
        or m == "gemini-nano-banana-2"
    )

def _resolve_edit_model(model: str) -> str:
    """
    处理编辑模型名映射（例如 SiliconFlow 的 Qwen-Image 编辑模型）。
    可通过环境变量 SILICONFLOW_IMAGE_EDIT_MODEL 覆盖默认映射。
    """
    override = os.getenv("SILICONFLOW_IMAGE_EDIT_MODEL", "").strip()
    if override:
        return override
    # 默认映射：Qwen/Qwen-Image -> Qwen/Qwen-Image-Edit
...
async def call_dalle_image_generation_async(
    ...
    DALL-E 图像生成
    """
    url = f"{_normalize_openai_base_url(api_url)}/images/generations".rstrip("/")
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(timeout)) as client:
        try:
            ...
            # 兼容两种常见返回结构：
            # 1) OpenAI 风格: {"data":[{"b64_json":...}]} / {"data":[{"url":...}]}
            # 2) SiliconFlow 部分模型: {"images":[{"url":...}]}
            b64_candidate = None
            image_url = None
            ...
            raise RuntimeError(f"未识别的图像生成响应结构: {str(data)[:500]}")
...
async def call_dalle_image_edit_async(
    ...
    DALL-E 图像编辑
    """
    url = f"{_normalize_openai_base_url(api_url)}/images/edits".rstrip("/")
    model = _resolve_edit_model(model)
    headers = {
        "Authorization": f"Bearer {api_key}",
    }
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(timeout)) as client:
        try:
            ...
            raise RuntimeError(f"未识别的图像编辑响应结构: {str(data)[:500]}")
                
        except httpx.HTTPStatusError as e:
            log.error(f"HTTPError {e}")
            log.error(f"Response body: {e.response.text}")
            # SiliconFlow may not expose /images/edits. If so, fallback to a second-generation step.
            if e.response.status_code == 404 and "siliconflow" in (api_url or "").lower():
                log.warning("[edit] /images/edits not available, fallback to /images/generations with edit prompt.")
                return await call_dalle_image_generation_async(
                    api_url=api_url,
                    api_key=api_key,
                    model=model,
                    prompt=prompt,
...
if __name__ == "__main__":
    ...
    async def _demo():
        ...
        API_KEY = os.getenv("UNIAPI_API_KEY") or os.getenv("SILICONFLOW_API_KEY") or os.getenv("DF_API_KEY", "sk-xxx")
    ...
    async def _test_123_edit():
        ...
        # API_URL = "http://123.129.219.111:3000/v1"
        API_URL= "http://b.apiyi.com:16888/v1"
        API_KEY = os.getenv("UNIAPI_API_KEY") or os.getenv("SILICONFLOW_API_KEY") or os.getenv("DF_API_KEY", "sk-123456") 
